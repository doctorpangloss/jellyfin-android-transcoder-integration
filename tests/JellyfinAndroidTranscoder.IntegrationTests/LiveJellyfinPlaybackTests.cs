...
public sealed class LiveJellyfinPlaybackTests
{
    ...
    public async Task LiveManualJellyfinPlaysRemoteAndroidHlsPastSegmentWindow()
    {
        ...
        Assert.True(seekResult.CurrentTime >= 34, $"Expected live browser playback to continue after seek, got {seekResult.CurrentTime:0.0}s.");
        Assert.True(
            seekResult.SeekResumeMilliseconds < 10_000,
            $"Expected live browser playback to resume within 10s after seek, took {seekResult.SeekResumeMilliseconds / 1000:0.0}s.");
